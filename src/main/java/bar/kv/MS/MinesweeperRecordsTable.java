...
public class MinesweeperRecordsTable extends JComponent {
    ...
    public MinesweeperRecordsTable(PageManager pageManager) {
        ...
        try {
            ...
            for (String str : strings) {
                scores.add(gson.fromJson(str, MinesweeperRecordsTable.ScoreString.class));
            }
        } catch (IOException e) {
    ...
    public void addScoreString(int bombsNum, int fieldSize, long time) {
        scores.add(new MinesweeperRecordsTable.ScoreString(bombsNum, fieldSize, time));
        File table = new File("src\\main\\resources\\krMinesweeperTable\\Table.json");
        if (!table.exists()) {
            try {
                Files.createFile(table.toPath());
        ...
        Gson gson = new Gson();
        try {
            Files.writeString(table.toPath(), gson.toJson(new MinesweeperRecordsTable.ScoreString(bombsNum, fieldSize, time)) + "\n", StandardOpenOption.APPEND);
        } catch (IOException e) {
    ...
    private class ScoreString implements Comparable<MinesweeperRecordsTable.ScoreString> {
        ...
        @Override
        public int compareTo(MinesweeperRecordsTable.ScoreString o) {
